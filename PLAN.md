...
UnidentifiedItemSighting (id, userId FK, characterId FK?, screenshotId FK,
                  ...
                  // cluster (via the normal add-item search flow, pre-seeded with the
                  // crop as a visual reference) sets resolved=true + resolvedItemCatalogId
                  // on every sighting in that cluster, creates the UserTrackedItem row
                  // (running the same identifiableByIcon collision check as any other
                  // add), and backfills CharacterItemCount for every character
                  // represented in the cluster's sightings — the user shouldn't have to
                  // re-upload something the app already saw
```
...
**Discovery worklist for everything else** (added 2026-07-09): asking a new user to identify their full inventory up front was considered and rejected — MapleStory has far too large an item set for that to be anything but overwhelming at the exact moment a new user needs the app to feel easy. Instead, unmatched icons are collected passively as a byproduct of normal uploads (see the `unmatched_items` schema field in "Screenshot ingestion" below and the `UnidentifiedItemSighting` table in the data model above) and surfaced as a self-paced, low-visibility worklist on the Items page instead of a gate blocking first use. Resolving a worklist entry routes through this exact same add-item search flow described above (candidate icons for visual disambiguation, `needsCategoryReview` fallback, `iconRefKey` fallback for unmatched newest content) — discovery only changes *where the prompt to identify something comes from* (a real sighting in the user's own inventory, surfaced opportunistically) rather than duplicating how identification itself works.
...
- **No manual screenshot-type selector** — one drag-and-drop zone accepts anything. One Claude call per image using a discriminated JSON schema (forced via tool-use):
  ...
  ```
  `unmatched_items` (added 2026-07-09) covers every icon in the grid that doesn't match anything in the reference-icon set — feeds the discovery worklist (see "Item catalog & icon references" and `UnidentifiedItemSighting` above) rather than being dropped on the floor. This rides along in the same call, not a second one: no extra Claude cost for surfacing "here's something you haven't told us about yet." `likely_category` is a soft grouping hint only, not a claimed identity.
