...
| DB access (Kotlin) | **Plain JDBC** (`java.sql.Connection`/`PreparedStatement`, no query-builder library) + **Flyway** for migrations | Revised 2026-07-11 — originally Exposed (JetBrains' Kotlin SQL library), swapped out deliberately in favor of raw parameterized SQL: no ORM/query-builder abstraction between the code and what's actually sent to Postgres, at the cost of writing result-set mapping and connection/resource handling by hand. Flyway is unaffected by this choice — it was never coupled to Exposed, it's a separate concern (schema migration, not querying) that plain JDBC still needs exactly as much as Exposed did. |
...
## Data model (illustrative schema — plain SQL/JDBC, not Exposed table objects; see tech stack table)
...
Either way, this stays a one-time seeding task, not a live per-request fetch — download once during the M1 migration, store in S3 as `iconRefKey`, and the vision prompt reads from S3 from then on.
...
1. **M0 — Scaffold both services + AWS infrastructure**: provision the VPC (public subnet for the ALB, private subnets for ECS tasks + RDS), security groups, ECR repo, RDS instance, and ECS cluster/service/task definition via Terraform. Ktor project (routing skeleton, `Authentication`/`jwt` wired to Clerk's JWKS, Exposed+Flyway pointed at RDS), Next.js project (Clerk web SDK, calling one Ktor health-check endpoint through the ALB). Deploy both (ECS Fargate + Vercel) and confirm an authenticated round-trip end-to-end before building features. This milestone is the heaviest lift in the whole plan given the infrastructure involved — budget real time for it.
   ...
   - **CI/CD pipeline built 2026-07-10** (GitHub Actions, closing the "fully manual deploy" gap): `infra/github-oidc.tf` adds a GitHub OIDC provider + IAM role scoped to this repo's `master` branch (no long-lived AWS keys in GitHub), plus a `deployment_circuit_breaker` on the ECS service so a bad deploy auto-reverts. `.github/workflows/backend-verify.yml` (reusable ktlint/detekt/test), `ci.yml` (paths-filtered PR/push gate, proven green via a throwaway smoke-test PR — which also caught two real bugs: `dorny/paths-filter` needing `pull-requests: read` on `GITHUB_TOKEN`, and `backend/gradlew` having silently lost its executable bit in git because `core.fileMode=false` on this drvfs-mounted workspace), and `deploy-backend.yml` (git-SHA-tagged image + new ECS task-definition revision per deploy, `workflow_dispatch`-only for now) are all landed on `master`. **Not yet exercised**: `deploy-backend.yml` has never actually been dispatched — paused because the AWS new-account compute hold (previous bullet) was still active as of 2026-07-10, and dispatching it would just hit the same task-placement failure on the final step. Next session: check `aws ecs describe-services --cluster maplestorage-cluster --services maplestorage-backend` first: if a task is finally running, dispatch `deploy-backend.yml` manually (`gh workflow run deploy-backend.yml`), confirm it goes healthy, then add the `push`-to-`master` trigger once 2-3 manual runs are clean. **Hold timeline**: first block event was 2026-07-10 18:17 UTC, still blocked as of 19:34 UTC same day — only a few hours old as of that check, well within the normal ~24h self-resolve window. Don't open an AWS Support case before ~2026-07-11 18:17 UTC; check the AWS Health Dashboard (health.aws.amazon.com) and account email for any verification ask first, since resolving that directly is faster than waiting.
2. **M1 — Data model + token catalog seed**: Exposed tables above; Flyway migration seeding all 6 `TokenCatalog` rows, including the one-time icon sourcing described in "Token catalog" above (download all 6 from `maplestorywiki.net` and upload to S3 as `iconRefKey`). No per-user tracking table to wire up — every user sees every row by definition.
