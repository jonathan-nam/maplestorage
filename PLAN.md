...
You're a long-time GMS MapleStory player running 15+ characters, and lack any unified view of how much of each item — Eternal-set boss tokens, consumable potions, enhancement scrolls, Etc-tab drop materials — you've accumulated *across* all characters. Today that means logging into every character individually — this app replaces that with screenshot uploads parsed by a vision-capable LLM into a unified item dashboard. Some item data (expiration dates, visually ambiguous items) needs a second screenshot type beyond the main grid — see "Tooltip screenshots" below — but deliberately stays within the same simple screenshot-in, structured-data-out model rather than requiring anything more exotic; an earlier video-capture design for this was considered and dropped (2026-07-10) specifically because it broke that simplicity.
...
CharacterItemCount    (characterId FK, itemCatalogId FK, quantity: int,
                  ...
                  // added 2026-07-08, populated via tooltip screenshots (see "Tooltip
                  // screenshots" below) — expirationNeedsReview is set, not silently
                  // overwritten, when a newly-detected date conflicts with an existing one
                  // for the same row (e.g. two stacks of the same item with different
                  // expiration dates)
...
UserTrackedItem   (userId FK, itemCatalogId FK, addedAt, identifiableByIcon: boolean)
                  ...
                  // resolve — every count for that item instead comes from a tooltip
                  // screenshot (see "Tooltip screenshots" below), which reads quantity and
                  // identifying text off the same single image
...
Screenshots       (id, userId, characterId?, type: INVENTORY|UNRECOGNIZED|TOOLTIP,
                  ...
                  // TOOLTIP (renamed 2026-07-10 from TOOLTIP_CAPTURE/EXPIRATION_TOOLTIP,
                  // added 2026-07-08) — a screenshot of an item's hover tooltip, uploaded
                  // through the exact same drag-and-drop/paste flow as a plain inventory
                  // grid, not a special capture mode. Serves three purposes: capturing an
                  // expirable item's date, capturing full item data (identity + quantity)
                  // for an item flagged identifiableByIcon=false, and identifying an
                  // UnidentifiedItemSighting — see "Tooltip screenshots" below

UnidentifiedItemSighting (id, userId FK, characterId FK?, screenshotId FK,
                           croppedIconKey: string, quantity: int,
                           likelyCategory: string?, capturedAt,
                           clusterKey: string?, resolved: boolean,
                           resolvedItemCatalogId FK?)
                  // added 2026-07-09 for the discovery-worklist flow: one row per icon
                  // a grid-pass vision call saw but couldn't match against the user's
                  // UserTrackedItem reference set (see the `unmatched_items` schema field
                  // below). croppedIconKey is an S3 crop taken server-side from the
                  // original screenshot using the model's returned bounding box — no
                  // extra Claude call. likelyCategory is a soft display hint only (never
                  // written to ItemCatalog.category directly), since asking the model to
                  // guess a specific item name is exactly the kind of ungrounded guess
                  // this app otherwise avoids (see identifiableByIcon above) — category
                  // is a much safer guess than identity. clusterKey groups sightings that
                  // look like the same item (rough perceptual-hash match on the crop, not
                  // a ground-truth identity claim) purely so the worklist shows one row
                  // per apparent item instead of one per sighting; a wrong grouping just
                  // means a slightly messier worklist row, not a data-integrity problem,
                  // so this doesn't need the rigor the digit-recognition spike needed —
                  // there's a human review step downstream either way. Resolving a
                  // cluster works two ways (see "Tooltip screenshots" below): the normal
                  // add-item search flow if the user already knows the name, or a tooltip
                  // screenshot of the item (crop shown alongside as a visual reference)
                  // if they don't, which reads the real name automatically instead of
                  // requiring the user to already have it. Either path sets resolved=true
                  // + resolvedItemCatalogId on every sighting in that cluster, creates the
                  // UserTrackedItem row (running the same identifiableByIcon collision
                  // check as any other add), and backfills CharacterItemCount for every
                  // character represented in the cluster's sightings — the user shouldn't
                  // have to re-upload something the app already saw
```
...
**New user default tracking** (added 2026-07-09): every new `Users` row is created with a `UserTrackedItem` row auto-inserted for each `ItemCatalog` row that's both `redemptionTracked=true` and `confirmed=true` — currently the 6 confirmed Eternal tokens (Akechi stays out until its `confirmed` flag flips, same reasoning as not defaulting anyone into an unverified item). This exists to solve a real cold-start problem, not just as a nicety: without *something* pre-tracked, a first upload hits an empty reference-icon set and identifies nothing at all — a broken first impression, not a merely suboptimal one. It's deliberately a narrow, fixed default (endgame boss tokens), not an attempt to guess what a given player broadly cares about across MapleStory's huge item set — that broader question is answered by the discovery worklist below, from the player's own inventory, not an app-wide assumption baked into a seed script.

**Discovery worklist for everything else** (added 2026-07-09): asking a new user to identify their full inventory up front was considered and rejected — MapleStory has far too large an item set for that to be anything but overwhelming at the exact moment a new user needs the app to feel easy. Instead, unmatched icons are collected passively as a byproduct of normal uploads (see the `unmatched_items` schema field in "Screenshot ingestion" below and the `UnidentifiedItemSighting` table in the data model above) and surfaced as a self-paced, low-visibility worklist on the Items page instead of a gate blocking first use. Resolving a worklist entry offers two paths (revised 2026-07-10, see "Tooltip screenshots" below): the same add-item search-by-name flow described above, for a user who already knows what the item is, or a tooltip screenshot of it — which reads the real name automatically, so the user never has to already know it. Either path ends the same way: candidate icons for visual disambiguation, `needsCategoryReview` fallback, `iconRefKey` fallback for unmatched newest content.
...
- **No manual screenshot-type selector** — one drag-and-drop zone accepts anything. One Claude call per image using a discriminated JSON schema (forced via tool-use):
  ```json
  {
    "screenshot_type": "inventory" | "unrecognized" | "tooltip",
    "character_hud": { "name": string|null, "level": number|null } | null,
    "inventory_items": [{ "item_name": string, "quantity": number }] | null,
    "unmatched_items": [{ "bounding_box": [number, number, number, number], "quantity": number,
                           "likely_category": "EQUIP"|"USE"|"ETC"|"SETUP"|"CASH"|"DEC"|"UNKNOWN" }] | null,
    "tooltip_read": { "item_name": string, "quantity": number|null, "expires_at": string|null } | null
  }
  ```
  `unmatched_items` (added 2026-07-09) covers every icon in the grid that doesn't match anything in the reference-icon set — feeds the discovery worklist (see "Item catalog & icon references" and `UnidentifiedItemSighting` above) rather than being dropped on the floor. This rides along in the same call, not a second one: no extra Claude cost for surfacing "here's something you haven't told us about yet." `likely_category` is a soft grouping hint only, not a claimed identity. `tooltip_read` (added 2026-07-08, folded into this schema 2026-07-10) is populated only when `screenshot_type = "tooltip"` — see "Tooltip screenshots" below.
...
**Problem (added 2026-07-08, generalized 2026-07-09)**: some item data only ever renders in the per-item hover tooltip, never in the plain inventory grid — for three reasons:
...
2. **Icon-ambiguous items** (added 2026-07-09): some items a user tracks are visually indistinguishable from each other by icon alone (`UserTrackedItem.identifiableByIcon=false` — see "Item catalog & icon references"). For these, the grid pass can't safely produce *any* count, not just a missing date — the tooltip's name/description text is the only thing that disambiguates which item is which.
3. **Discovery-worklist identification** (added 2026-07-10): an `UnidentifiedItemSighting` cluster has a cropped icon but no name — same underlying problem, the tooltip is the only place that name exists.

**Reconsidered and simplified 2026-07-10**: the original design for this built a dedicated `getDisplayMedia()` video-capture pipeline — client-side frame-diffing over a live screen recording, extracting a keyframe per hovered item, so one recording session could batch several tooltip reads at once. That was dropped after weighing it against how the app is actually used: all three cases above are infrequent (a handful of items at a time, not a routine part of every upload), so the batching a video session enabled wasn't worth what it cost in friction — an OS-level screen-recording permission grant, picking the right window, and choreographing mouse movement in-game, all meaningfully heavier than anything else in the app. Manual data entry was considered as the alternative and also rejected — it would work, but undermines the app's actual pitch (vision reads the data, the user doesn't) for exactly the items that need it most.

**What replaced it: a tooltip screenshot is just another screenshot.** Hover the item in-game, take a screenshot the same way already taught for the main grid (`Win+Shift+S` / `Cmd+Shift+4`), drag or paste it into the exact same upload zone as always. The discriminated schema (see "Screenshot ingestion" above) gains a third `screenshot_type: "tooltip"` case, populating `tooltip_read: { item_name, quantity, expires_at }`. A single screenshot is genuinely sufficient, not a compromise — confirmed directly against `reference-images/expiring.png`, where the grid stays visible behind an open tooltip, so the same frame that shows the tooltip's identifying/date text also shows the hovered item's stack-count badge from the grid underneath. Identity, quantity, and expiration date (when present) all come from one image, exactly what the video approach was trying to guarantee, without needing video to get there.

This costs nothing new to build beyond the schema case itself: no new upload transport, no new concurrency model, no new client-side capture code, no new usage-tracking mechanism — a tooltip screenshot flows through the identical drag-and-drop/paste/presigned-S3 pipeline as a grid screenshot, and bulk-drop works on it for free (take a few tooltip screenshots for a few items, drop them all in together).

**Character attribution**: a tooltip screenshot has no HUD to cross-check against, same as a tightly-cropped grid screenshot — no special-casing needed, it falls through to the same `NEEDS_REVIEW`-with-manual-picker path that already exists for any HUD-less upload (see "Screenshot ingestion" above). Recommend pinning a character first in the UI copy, but don't hard-block on it — the existing fallback already covers it.

**Resolution, by which of the three problems triggered it**:
- `expires_at` present → upserts `CharacterItemCount.expiresAt` for the resolved character + item, flagging `expirationNeedsReview` on conflict, same as before.
- Matches an `identifiableByIcon=false` item → both `quantity` and identity resolve from this one read, same as always intended for this case.
- Matches an `UnidentifiedItemSighting` cluster → `tooltip_read.item_name` is searched live against `maplestory.io` (same as any add-item search), shown as candidates for the user to confirm, resolving the cluster, creating the `UserTrackedItem` row (running the `identifiableByIcon` collision check as usual), and backfilling `CharacterItemCount` for every character in that cluster's prior sightings.
...
2. **M1 — Data model + catalog seed**: Exposed tables above; Flyway migration + a seed script for the 6 confirmed `redemptionTracked=true` `ItemCatalog` rows (+ Akechi placeholder), categorized `ETC`. Also wires the default-tracking step (added 2026-07-09, see "Item catalog & icon references"): first-time `Users` row creation auto-inserts `UserTrackedItem` for every `confirmed=true` row in that seed set, so a new user's first upload has a non-empty reference-icon set instead of matching nothing.
...
10. **M9 — Tooltip screenshots**: Flyway migration for `ItemCatalog.expirable`, `UserTrackedItem.identifiableByIcon`, `CharacterItemCount.expiresAt`/`expirationNeedsReview`, and the `Screenshots.type` enum (`TOOLTIP`); extend the discriminated schema with the `tooltip_read` case (no new client-side capture code needed — a tooltip screenshot uploads through the same flow as any other image); Ktor parsing/matching logic writing `expiresAt` for `expirable` items, quantity/identity for `identifiableByIcon=false` items, and resolving `UnidentifiedItemSighting` clusters. See "Tooltip screenshots" above for the full design — considerably smaller in scope than the video-capture design it replaced (2026-07-10).
...
- After M1: create a fresh test user and confirm `UserTrackedItem` already contains the confirmed default token set before any upload happens — the Items page should show those rows (at 0/unset quantity) immediately on first login, not after a first successful parse.
...
- After M7's discovery-worklist path: upload a screenshot containing at least one item outside the default token set, confirm it surfaces as an `UnidentifiedItemSighting` with a correct crop and a plausible `likelyCategory`, then resolve it via the worklist and confirm `CharacterItemCount` backfills correctly without a re-upload.
...
- After M9: upload a tooltip screenshot of a known expiring item and confirm the resulting `expiresAt` matches what's visible in-game. Separately, upload one for an `identifiableByIcon=false` item and confirm both quantity and identity resolve from that single image with no grid-pass contribution needed. Separately, resolve a discovery-worklist entry via a tooltip screenshot and confirm it produces the same result as resolving the same item by name search.
