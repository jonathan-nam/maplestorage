...
internal fun lootFromClear(
    ...
    for ((dropId, pieces) in guaranteedDrops(bossCatalogId, difficulty)) {
        val already =
            PartyLoot
                .selectAll()
                .where {
                ...
                .not()
        if (already) continue
        // Dated inside the period the CLEAR is for, so a clear ticked for a past week files in that
        // week rather than this one. Today when the period is the one today falls in, which is the
        // ordinary case and the date a human would have typed.
        val droppedOn = if (on >= period && on < nextPeriod) on else period
        addLoot(partyId, LootedDrop(dropId, quantity = pieces), bossCatalogId, droppedOn, now, fromClear = true)
    }
}
